...
test.describe("Scripts and settings wiring", () => {
  ...
  test("settings exposes ready-state defaults and Pollinations image provider", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /Provedores de mídia/ }).click();
    await expect(page.getByRole("heading", { name: "Provedores de mídia" })).toBeVisible();
    await expect(page.getByRole("heading", { name: "Black Forest Labs" })).toBeVisible();
    await expect(page.getByRole("heading", { name: "Fal.ai" })).toBeVisible();
    await expect(page.getByRole("heading", { name: "Replicate" })).toBeVisible();
    await expect(page.getByRole("heading", { name: "Suno" })).toBeVisible();
    await expect(page.getByRole("heading", { name: "Udio" })).toBeVisible();
